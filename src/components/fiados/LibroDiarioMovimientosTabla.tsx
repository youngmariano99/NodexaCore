      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-400">
          ...
          <button
            ...
            className={`rounded-lg px-2.5 py-1 text-2xs font-semibold border transition-all ${
              filtroTipo === "todos"
        ...
        <table className="w-full border-collapse text-left text-xs">
          <thead>
            <tr className="border-b border-slate-800 bg-slate-950 text-slate-400">
              <th className="px-4 py-3 font-semibold">ID Mov.</th>
              <th className="px-4 py-3 font-semibold">Fecha y Hora</th>
              <th className="px-4 py-3 font-semibold">Cliente</th>
              <th className="px-4 py-3 font-semibold">Tipo</th>
              <th className="px-4 py-3 font-semibold">Comprobante / Detalle</th>
              <th className="px-4 py-3 font-semibold text-right">Monto ($)</th>
              <th className="px-4 py-3 font-semibold text-center">Estado Imputación</th>
              <th className="px-4 py-3 font-semibold text-center">Control Contable</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-800/60 text-slate-200">
            {movimientosFiltrados.length === 0 ? (
              ...
              movimientosFiltrados.map((m) => (
                <tr key={m.movimientoCcId} className="hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    {m.tipo === "pago" ? (
                      <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2 py-0.5 text-3xs font-semibold text-emerald-400">
                        <ArrowDownRight className="h-3 w-3" />
                        Pago / Cobro
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 rounded-full bg-amber-500/10 px-2 py-0.5 text-3xs font-semibold text-amber-400">
                        <ArrowUpRight className="h-3 w-3" />
                        Cargo / Venta
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3 font-mono text-2xs text-slate-300">
                    {m.numeroComprobante ?? m.comprobanteTipo}
                    {m.metodoPago && (
                      <span className="ml-2 rounded bg-slate-800 px-1.5 py-0.5 text-3xs text-slate-400 uppercase">
                        {m.metodoPago}
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-right font-mono text-sm font-bold text-slate-100">
                    {m.tipo === "pago" ? "−" : "+"}{formatearPrecio(m.monto)}
                  </td>
                  <td className="px-4 py-3 text-center">
                    {m.estadoImputacion === "total" && (
                      <span className="rounded bg-emerald-500/10 px-2 py-0.5 text-3xs font-semibold text-emerald-400 border border-emerald-500/30">
                        Total
                      </span>
                    )}
                    {m.estadoImputacion === "parcial" && (
                      <span className="rounded bg-amber-500/10 px-2 py-0.5 text-3xs font-semibold text-amber-400 border border-amber-500/30">
                        Parcial (${m.montoPendiente.toLocaleString("es-AR")})
                      </span>
                    )}
                    {m.estadoImputacion === "pendiente" && (
                      <span className="rounded bg-slate-800 px-2 py-0.5 text-3xs font-semibold text-slate-400">
                        Pendiente
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-center">
                    <span className="inline-flex items-center gap-1 text-3xs text-emerald-400 font-semibold">
