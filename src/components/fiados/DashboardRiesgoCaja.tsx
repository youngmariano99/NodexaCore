...
import { ESTADO_ACTUALIZAR_CONFIGURACION_RIESGO_INICIAL } from "@/services/fiados/actualizarConfiguracionRiesgoFiados";
...
export function DashboardRiesgoCaja({
  ...
  const abrirWhatsAppDeudor = (nombreCliente: string, telefono?: string | null, saldo?: number) => {
    if (!telefono) return;
    ...
    <div className="flex flex-col gap-8">
      ...
      <section className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-2xs text-slate-400">Resumen en tiempo real</span>
        </div>

        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {/* Card 1 */}
          <div className="flex flex-col gap-2 rounded-2xl border border-slate-800 bg-slate-900/90 p-5 shadow-lg relative group">
            <div className="flex items-center justify-between text-slate-400">
              <span className="text-xs font-semibold">Caja Total Recuperada</span>
            ...
            <div className="flex items-center gap-1 text-2xs text-slate-400">
              ...
              <span>Suma de cobros recibidos que ya volvieron a tu caja</span>
            </div>
          </div>

          {/* Card 2 */}
          <div className="flex flex-col gap-2 rounded-2xl border border-slate-800 bg-slate-900/90 p-5 shadow-lg">
            <div className="flex items-center justify-between text-slate-400">
              <span className="text-xs font-semibold">Total Vendido a Crédito</span>
            ...
            <div className="flex items-center gap-1 text-2xs text-slate-400">
              <HelpCircle className="h-3 w-3 text-slate-500 shrink-0" />
              <span>Suma histórica de todo lo fiado entregado</span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 rounded-2xl border border-slate-800 bg-slate-900/90 p-5 shadow-lg">
            <div className="flex items-center justify-between text-slate-400">
              <span className="text-xs font-semibold">Deuda Total en la Calle</span>
            ...
            <div className="flex items-center gap-1 text-2xs text-slate-400">
              <HelpCircle className="h-3 w-3 text-slate-500 shrink-0" />
              <span>Plata que está afuera pendiente de cobro</span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 rounded-2xl border border-slate-800 bg-slate-900/90 p-5 shadow-lg">
            <div className="flex items-center justify-between text-slate-400">
              <span className="text-xs font-semibold">% Fiado Recuperado</span>
              <span className="text-xs font-bold text-emerald-400">
                {situacionCaja.porcentajeFiadoCobrado}%
              </span>
            </div>
            <div className="w-full bg-slate-950 rounded-full h-3 overflow-hidden border border-slate-800 my-1">
              <div
            ...
            <div className="flex items-center gap-1 text-2xs text-slate-400">
              <HelpCircle className="h-3 w-3 text-slate-500 shrink-0" />
              <span>De cada $100 pesos fiasdos, ya cobraste ${situacionCaja.porcentajeFiadoCobrado}</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="flex flex-col gap-3 rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-2xs text-slate-400">
                Compara tu deuda en la calle contra tu facturación estimada para proteger tu caja.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {esComerciante && (
              <button
                ...
                className="flex items-center gap-1 rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-xs font-semibold text-slate-300 hover:bg-slate-700 transition-colors"
              >
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 pt-2">
          <div className="flex flex-col gap-1 rounded-xl bg-slate-950 p-3.5 border border-slate-800/80">
            ...
            <span className="text-3xs text-slate-500">
              {modoState === "automatico" ? "Calculado automático de ventas 30 días" : "Ingresado manualmente"}
            </span>
          </div>
          ...
          <div className="flex flex-col gap-1 rounded-xl bg-slate-950 p-3.5 border border-slate-800/80">
            ...
            <span className="text-3xs text-slate-500">Porcentaje máximo de fiado recomendado</span>
          </div>
          ...
          <div className="flex flex-col gap-1 rounded-xl bg-slate-950 p-3.5 border border-slate-800/80">
            ...
            <span className="text-3xs text-slate-500">Límite para no descalzar tu flujo de fondos</span>
          </div>
          ...
          <div className="flex flex-col gap-1 rounded-xl bg-slate-950 p-3.5 border border-slate-800/80">
            ...
            <span className="text-3xs text-slate-500">Porcentaje real de tu negocio fiado fuera</span>
          </div>
        </div>
        ...
        >
          {semaforoRiesgo.estadoAlertaRiesgo === "saludable" && (
            <p className="leading-relaxed">
              🟢 <strong>TODO EN ORDEN:</strong> Tu nivel de fiados representa el {semaforoRiesgo.deudaSobreFacturacionPct}% de tu facturación mensual, manteniéndose dentro del límite saludable del {semaforoRiesgo.topeDeudaTolerablePct}%. Podés otorgar crédito con tranquilidad.
            </p>
          )}
          {semaforoRiesgo.estadoAlertaRiesgo === "precaucion" && (
            <p className="leading-relaxed">
              🟡 <strong>PRECAUCIÓN DE CRÉDITO:</strong> Tu deuda en la calle está al {semaforoRiesgo.deudaSobreFacturacionPct}% de tu facturación mensual (cerca del límite recomendado de {formatearPrecio(semaforoRiesgo.montoMaximoRecomendado)}). Priorizá el cobro antes de fiar nuevos saldos.
            </p>
          )}
          {semaforoRiesgo.estadoAlertaRiesgo === "excesivo" && (
            <p className="leading-relaxed">
              🔴 <strong>ALERTA DE RIESGO EXCESIVO:</strong> Tu deuda en la calle ({formatearPrecio(situacionCaja.deudaTotalCalle)}) superó el monto máximo recomendado ({formatearPrecio(semaforoRiesgo.montoMaximoRecomendado)}). Te recomendamos no otorgar más crédito a fiado hasta recuperar liquidez.
            </p>
          )}
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <section className="flex flex-col gap-3 rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-xl">
          <div className="flex items-center gap-2 border-b border-slate-800 pb-3">
            <Users className="h-5 w-5 text-blue-400" />
            <h3 className="text-sm font-bold text-slate-100 uppercase tracking-wider">Control de Clientes</h3>
          </div>

          <div className="grid grid-cols-2 gap-3 text-xs">
            <div className="flex flex-col gap-0.5 rounded-xl bg-slate-950 p-3 border border-slate-800">
            ...
            <div className="flex flex-col gap-0.5 rounded-xl bg-slate-950 p-3 border border-slate-800">
              <span className="text-slate-400">Deuda Más Alta de 1 Cliente:</span>
              <span className="font-mono text-base font-bold text-amber-400">
        ...
        <section className="flex flex-col gap-3 rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-xl">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-slate-100 uppercase tracking-wider">Top 5 Deudores</h3>
            </div>
            ...
            <div className="flex flex-col gap-2.5">
              {top5Deudores.map((d, index) => (
                <div
                  ...
                  className="flex items-center justify-between rounded-xl bg-slate-950 p-3 border border-slate-800/90 text-xs"
                >
                  <div className="flex items-center gap-3">
                    <span className="flex h-6 w-6 items-center justify-center rounded-lg bg-slate-800 text-2xs font-bold text-slate-300">
                      #{index + 1}
                    </span>
                    <div className="flex flex-col">
                      <span className="font-bold text-slate-100">{d.nombre}</span>
                      <span className="text-3xs text-slate-400">
                        {d.diasSinPagar > 0 ? `${d.diasSinPagar} días sin pagar` : "Compra reciente"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {d.telefono && (
                      <button
                        type="button"
                        onClick={() => abrirWhatsAppDeudor(d.nombre, d.telefono, d.saldoDeudor)}
      ...
      {modalAjustesAbierto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm">
          <div className="flex w-full max-w-md flex-col gap-4 rounded-2xl border border-slate-800 bg-slate-900 p-6 text-slate-100 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            ...
            <form onSubmit={manejarGuardarConfiguracion} className="flex flex-col gap-4 text-xs">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="font-semibold text-slate-300">Modo de Facturación Referencia:</label>
                <select
                  value={modoState}
                  onChange={(e) => setModoState(e.target.value as "automatico" | "manual")}
                  className="rounded-xl border border-slate-700 bg-slate-950 p-2.5 text-slate-100 focus:border-emerald-500 focus:outline-none"
                >
              ...
              {modoState === "manual" && (
                <div className="flex flex-col gap-1.5">
                  <label className="font-semibold text-slate-300">Facturación Mensual Estimada ($):</label>
                  <input
                    type="number"
                    step="1000"
                    min="0"
                    ...
                    className="rounded-xl border border-slate-700 bg-slate-950 p-2.5 font-mono text-slate-100 focus:border-emerald-500 focus:outline-none"
                  />
                  <span className="text-3xs text-slate-400">
              ...
              {/* Tope Tolerable % */}
              <div className="flex flex-col gap-1.5">
                <label className="font-semibold text-slate-300">Tope de Deuda Tolerable (%):</label>
                <input
                  type="number"
                  step="1"
                  min="1"
              ...
              <div className="flex justify-end gap-2 border-t border-slate-800 pt-4 mt-2">
                ...
                <button
                  type="submit"
                  disabled={isPending}
                  className="flex items-center justify-center min-w-[100px] rounded-xl bg-emerald-500 px-4 py-2 font-bold text-slate-950 hover:bg-emerald-400"
                >
