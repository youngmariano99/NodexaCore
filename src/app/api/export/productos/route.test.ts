...
vi.mock("@/repositories/productosRepository", async () => {
  ...
  return { ...actual, obtenerTodosLosProductosActivos: vi.fn() };
});
...
function crearRequest(query = ""): NextRequest {
  return new NextRequest(`http://localhost:3000/api/export/productos${query}`);
}
...
describe("GET /api/export/productos", () => {
  ...
  it("retorna 403 con NX-SYS-003 cuando el solicitante es empleado", async () => {
    ...
    expect(obtenerTodosLosProductosActivos).not.toHaveBeenCalled();
  });
  ...
  it("resuelve el cliente_id únicamente desde la sesión, nunca desde la query string", async () => {
    const solicitanteBuilder = crearBuilder({ data: { rol: "comerciante", cliente_id: CLIENTE_ID }, error: null });
    const supabaseMock = { ...mockearSesion({ id: AUTH_USER_ID }), from: vi.fn(() => solicitanteBuilder) };
    vi.mocked(crearClienteSupabaseServidor).mockResolvedValue(supabaseMock as never);
    vi.mocked(obtenerTodosLosProductosActivos).mockResolvedValue({ ok: true, data: [] });

    await GET(crearRequest("?formato=json&cliente_id=otro-tenant-cualquiera"));

    expect(obtenerTodosLosProductosActivos).toHaveBeenCalledWith(expect.anything(), CLIENTE_ID);
  });

  it("responde CSV con Content-Type y Content-Disposition correctos (formato=csv)", async () => {
    const solicitanteBuilder = crearBuilder({ data: { rol: "comerciante", cliente_id: CLIENTE_ID }, error: null });
    const supabaseMock = { ...mockearSesion({ id: AUTH_USER_ID }), from: vi.fn(() => solicitanteBuilder) };
    vi.mocked(crearClienteSupabaseServidor).mockResolvedValue(supabaseMock as never);
    vi.mocked(obtenerTodosLosProductosActivos).mockResolvedValue({ ok: true, data: [PRODUCTO_EJEMPLO] });
    ...
    expect(texto).toContain("sku,nombre,categoria,precio,stock_actual,publicado");
    expect(texto).toContain("SKU-001");
  });
  ...
  it("retorna 500 con el código del repositorio si la consulta paginada falla", async () => {
    ...
    const supabaseMock = { ...mockearSesion({ id: AUTH_USER_ID }), from: vi.fn(() => solicitanteBuilder) };
    vi.mocked(crearClienteSupabaseServidor).mockResolvedValue(supabaseMock as never);
    vi.mocked(obtenerTodosLosProductosActivos).mockResolvedValue({ ok: false, error: "NX-SYS-001" });
