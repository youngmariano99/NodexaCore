...
export function obtenerSubdominioDesdeHost(host: string): string | null {
  const hostLimpio = host.split(":")[0].toLowerCase();

  if (DOMINIOS_PRINCIPALES.includes(hostLimpio)) {
    return null;
  }
  ...
  if (partes.length > 1) {
    ...
    if (!["www", "app", "admin", "api"].includes(primerSegmento)) {
 ...
 * 1. Resuelve subdominios y dominios personalizados para el Catálogo Web de forma dinámica.
 * 2. Si aplica, reescribe internamente la ruta hacia /c/[subdominio] manteniendo la URL visible inalterada.
 * 3. Para las rutas privadas/admin del sistema, valida la sesión de Supabase y políticas de acceso.
 */
export async function middleware(request: NextRequest) {
  ...
  // 1. Si el request viene desde un subdominio comercial o dominio personalizado del Catálogo Web:
  if (subdominioOHost && !pathname.startsWith("/c/")) {
    ...
    // Validar cliente activo y módulo catalogo_web activo en la base de datos
    ...
    // Reescribir la URL internamente manteniendo la URL del navegador inalterada (Criterio de Aceptación)
  ...
  // 2. Control de sesión y acceso a rutas de administración y panel interno
  ...
  const esRutaAdmin = PREFIJOS_RUTAS_ADMIN.some((prefijo) => pathname.startsWith(prefijo));
